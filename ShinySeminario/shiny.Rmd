...
- <p style="text-align: justify;">JavaScript el lenguaje de programación web no esta especializado en el análisis estadístico por lo que dificulta la generación de los reportes.</p>
...
<p style="text-align: justify;"> Permite un análisis flexible e interactivo de los resultados, con esto se quiere decir que el usuario no solo puede ver los reportes (gráficas, tablas, mapas, etc), mas aún el usuario puede modificaros, introduciendo nuevos datos, texto, seleccionando rangos, entre otros input o entradas, que por medio de widgets shiny permite al usuario comunicarse con el servidor.</p>
...
ui<-shinyUI(fluidPage(
  ...
  sidebarLayout(
    ...
    # Mostrar la grafica de la distrubucion generada
...
```{r server}
server<-shinyServer(function(input, output) {
  # Codigo para generar un histogram. El codigo esta dentro de 
  # de la función renderPlot para indicar que:
  #
  #  1) Es "rectivo", esto es se debe ejecutar automaticamente
  #     cada vez que una entrda cambie.
  #  2) Genera una slida de tipo plot (gráfica)

  output$distPlot <- renderPlot({
    x    <- faithful[, 2]  # Datos de Old Faithful Geyser
    barras <- seq(min(x), max(x), length.out = input$barras + 1)

    # Crea un istograma con el numero de barras especificado.
